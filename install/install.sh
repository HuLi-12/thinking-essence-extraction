...
curl -fsSL "$REPO_URL/raw/$BRANCH/skills/thinking-essence-extraction/SKILL.md" \
  -o "$TARGET/.codex/skills/thinking-essence-extraction/SKILL.md"
...
for file in checkpoints.json; do
  curl -fsSL "$REPO_URL/raw/$BRANCH/skills/thinking-essence-extraction/evals/$file" -o "$TARGET/.codex/skills/thinking-essence-extraction/evals/$file"
done
...
curl -fsSL "$REPO_URL/raw/$BRANCH/wrappers/claude/subagent.md" -o "$TARGET/CLAUDE.md"
